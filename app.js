process.on("unhandledRejection", () => {}); // let per-request handler show error page
...
// });
const store = MongoStore.create({
  mongoUrl: dbUrl, // ← back to mongoUrl, not clientPromise
  crypto: { secret: process.env.SECRET },
  touchAfter: 24 * 60 * 60,
  autoRemove: "native",
  mongoOptions: {
...
store.on("error", (err) => console.error("store error:", err.message));
// ← ADD THIS LINE:
store.client?.catch?.(() => {});
...
app.use(async (req, res, next) => {
  try {
    ...
    next(
      new ExpressError(
        500,
        "Could not connect to database. Please try again in a moment.",
      ),
    );
  }
});
